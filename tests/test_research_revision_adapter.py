...
def test_targets_are_explicit_and_narrow():
    ...
    assert adapter.GOVERNED_RESEARCH_TARGETS["PMOD-P2-B2"].module_path.endswith("pmod_p2_b2_certification.py")
    assert adapter.GOVERNED_RESEARCH_TARGETS["IR11-P3-B2"].module_path.endswith("ir11_p3_normalization_b2.py")
...
def test_unapproved_job_is_rejected(tmp_path):
    with pytest.raises(RuntimeError, match="not approved"):
        adapter.run_governed_revision(
            repo_root=tmp_path,
            work_root=tmp_path,
            runner_job_id="NOT-APPROVED",
            research_sha="1" * 40,
        )
...
def test_run_governed_revision_records_provenance(monkeypatch, tmp_path):
    ...
    result = adapter.run_governed_revision(
        repo_root=tmp_path,
        work_root=tmp_path,
