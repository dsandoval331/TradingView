...
def main():
    ...
    # C2 VWAP PENETRATION
    #
    # Positive values mean C2 moved through VWAP before reclaiming
    # back to the favorable side by the close.
    # -------------------------------------------------------------------------
    ...
    # -------------------------------------------------------------------------
    # C1 -> C2 CLOSE PROGRESS
    #
    # Positive always means favorable progress relative to signal direction.
    # -------------------------------------------------------------------------
    ...
    if len(unique_dates) < 2:
        raise RuntimeError(
            "Insufficient dates for temporal split."
        )
    ...
    # PARQUET-SAFE EXPORT
    #
    # pd.qcut() produces pandas Categorical/Interval values.
    # PyArrow cannot reliably serialize pandas Interval extension columns.
    #
    # Keep the in-memory categorical buckets for analysis, but convert only
    # the exported copy to ordinary string labels.
    # =========================================================================
    ...
    print(
        "AE2 C2-SAFE PREDICTOR SCREEN V1"
    )
