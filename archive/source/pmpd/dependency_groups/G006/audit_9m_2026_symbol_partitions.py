...
def has_symbol_token(path, sym):
    parts=re.split(r"[^A-Z0-9]+", str(path).upper())
    return sym in parts
...
print("UNMAPPED_OR_AMBIGUOUS_FILES =",len(unmapped))
print("SYMBOLS_WITH_MULTIPLE_2026_FILES =",len(multi))

# Inspect formats and lightweight parquet metadata where available.
fmt=defaultdict(int)
parquet_schema_counts=defaultdict(int)
...
report={
 "protocol":"PMPD_V5_9M_2026_SYMBOL_PARTITION_AUDIT_V1",
 ...
 "unmapped_or_ambiguous_count":len(unmapped),
 "unmapped_or_ambiguous":unmapped[:200],
...
op=ROOT/"pmpd_v5_9m_2026_symbol_partition_audit_v1.json"
...
print("9M_SYMBOL_PARTITION_AUDIT_GATE=PASS")
