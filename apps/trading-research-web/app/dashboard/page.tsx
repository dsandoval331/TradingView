...
export default async function DashboardPage() {
  ...
  const renderGroup = (eyebrow: string, title: string, description: string, items: Strategy[]) => <section className="portfolioSection"><div className="sectionHeader"><div><p className="eyebrow">{eyebrow}</p><h2>{title}</h2><p className="sectionDescription">{description}</p></div><span>{items.length} project{items.length === 1 ? "" : "s"}</span></div><div className="strategyGrid">{items.map(renderCard)}</div></section>;
  ...
  return <main>
    ...
    {queryError ? <section className="alertPanel"><strong>Dashboard query failed.</strong><span>{queryError.message}</span></section> : <>
      ...
      {renderGroup("INDICATORS & STRATEGIES", "Indicators & strategies", "TradingView indicators, strategy research, validation, and model development.", indicatorStrategies)}
      {renderGroup("TOOLS", "Tools", "Shared research and execution-support systems that sit alongside the trading strategies.", tools)}
      {renderGroup("TRADE REVIEWS", "Trade reviews", "Executed-trade reconstruction, review, trade-health analysis, and trader-development research.", tradeReviews)}
    </>}
  </main>;
}
