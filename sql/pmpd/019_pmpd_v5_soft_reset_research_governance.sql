...
--   3) Move project_state to the current V5 research position without deleting
--      or rewriting the authoritative PMPD-RM-1.0 factor inventory.
--   4) Preserve V4 as the frozen benchmark.
--
-- IMPORTANT
--   * This migration does NOT alter frozen V4 signal logic.
--   * This migration does NOT replace the 205-factor PMPD-RM-1.0 inventory.
--   * V5 factor remapping/versioning should be a later migration after 9G closes.
--   * Idempotent: phases are upserted; decisions are inserted only if absent.
-- ============================================================================
...
phases(phase_code, phase_name, objective, status, sequence_order,
    ...
    ('9B','Existing-Evidence Inventory',
     ...
     '9C',
     'Methodology may transfer; conclusions/thresholds do not.'),
    ...
    ('9G','Large-Sample Baseline / Event Population',
     ...
     '9H',
     'Current substep: 9G-3B/3C. Alpha 0.2.1 corrected full-universe run pending local completion/upload.'),
...
insert into public.program_phases (
    strategy_id, phase_code, phase_name, objective, status, sequence_order,
...
select pmpd.strategy_id, d.title, d.decision, d.rationale, d.evidence,
...
set
    ...
    historical_dataset_status = 'PMPD_112_V1 / 2025 CERTIFIED; V5 STRUCTURAL AUDIT ACTIVE',
    last_decision = 'V5 Alpha 0.2.1 export/data-integrity hardening run pending; close 9G before exposing factor research to outcomes.',
    metadata_json =
...
begin
    ...
    if not exists (
        select 1 from public.project_state
        where strategy_id = v_strategy_id
