...
GOVERNED_RESEARCH_TARGETS: dict[str, GovernedResearchTarget] = {
    "PMOD-P2-B2": GovernedResearchTarget(
        runner_job_id="PMOD-P2-B2",
        module_path="research_runner/jobs/pmod_p2_b2_certification.py",
    ),
    "IR11-P3-B2": GovernedResearchTarget(
        runner_job_id="IR11-P3-B2",
        module_path="research_runner/jobs/ir11_p3_normalization_b2.py",
    ),
}
...
def run_governed_revision(
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="governed-research-") as td:
        module_file = materialize_module(
            repo_root,
            verified_sha,
            target.module_path,
            Path(td) / Path(target.module_path).name,
        )
        spec = importlib.util.spec_from_file_location(
            f"governed_{runner_job_id.lower().replace('-', '_')}", module_file
        )
    ...
    return {
        **result,
        "governed_research_sha": verified_sha,
        "governed_research_module": target.module_path,
