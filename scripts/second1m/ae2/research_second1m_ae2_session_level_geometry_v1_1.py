...
def main() -> None:
    ...
    # Exact parity with the frozen session-level implementation:
    # raw three-level span normalized by entry price. In this dataset,
    # entry_price == c2_close, but entry_price is the canonical denominator.
    entry = pd.to_numeric(g["entry_price"], errors="coerce").to_numpy()
    g["level_cluster_span_pct_calc"] = (max_level - min_level) / entry * 100.0
