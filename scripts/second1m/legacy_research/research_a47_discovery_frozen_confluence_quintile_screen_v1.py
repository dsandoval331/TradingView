...
FEAT=ROOT/"ae2_level_confluence_v1"/"a47_entry_area_confluence_features_v1.parquet"
OUTDIR=ROOT/"ae2_level_confluence_v1"; OUTDIR.mkdir(parents=True,exist_ok=True)
THR=OUTDIR/"a47_discovery_quintile_thresholds_v1.csv"
OUT=OUTDIR/"a47_predeclared_quintile_screen_v1.csv"
BINARY={"FAVORABLE_FIRST","ADVERSE_FIRST"}

# Keep the family compact. The two summaries are primary; individual references are robustness-only.
FEATURES=[
 ("a47_center_to_nearest_tech_abs_pct","PRIMARY_NEAREST"),
 ("a47_center_to_mean_tech_abs_pct","PRIMARY_MEAN"),
 ("a47_center_to_ema9_abs_pct","ROBUSTNESS_EMA9"),
 ("a47_center_to_ema20_abs_pct","ROBUSTNESS_EMA20"),
 ("a47_center_to_vwap_abs_pct","ROBUSTNESS_VWAP"),
]
...
def main():
    ...
    for col,role in FEATURES:
        ...
        if len(edges)!=6: raise RuntimeError(f"{col}: expected 5 quintiles, got {len(edges)-1}")
        ...
        x[col+"__A47Q"]=pd.cut(x[col],bins=bins,labels=["Q1","Q2","Q3","Q4","Q5"],include_lowest=True)
    pd.DataFrame(thresholds).to_csv(THR,index=False)

    rows=[]
    print("="*125)
    print("A47.3 - PREDECLARED DISCOVERY-FROZEN ENTRY-AREA CONFLUENCE QUINTILE SCREEN")
    print("="*125)
    print(f"Historical Preferred binary events: {len(x):,}")
    print("Lower quintile = tighter structural-center proximity to the technical reference(s).")
    print("Primary summaries were frozen before outcome exposure; individual EMA/VWAP measures are robustness-only.")
    ...
    for period in ["DISCOVERY","VALIDATION"]:
        ...
        for col,role in FEATURES:
            ...
            for q in ["Q1","Q2","Q3","Q4","Q5"]:
                h=z[z[col+"__A47Q"].eq(q)]; n,r=stats(h); lift=r-br if n else np.nan
    ...
    print("\nPRIMARY REPLICATION CELLS: same sign and |lift| >=3pp in BOTH periods")
    print("-"*125)
    for col,role in FEATURES[:2]:
        ...
        for q in ["Q1","Q2","Q3","Q4","Q5"]:
            ...
            if np.sign(dl)==np.sign(vl) and abs(dl)>=3 and abs(vl)>=3:
                found=True
                print(f"{role} {q}: DISC {dl:+.2f}pp N={int(d.loc[q,'n'])} | VAL {vl:+.2f}pp N={int(v.loc[q,'n'])}")
        if not found: print(f"{role}: NONE")

    print(f"\nThresholds: {THR}")
    print(f"Screen:     {OUT}")
    print("RESULT: A47 QUINTILE SCREEN COMPLETE")
    print("No threshold optimization, regrouping, or prospective data used.")
    print("No Candidate Model V1 modified.")
