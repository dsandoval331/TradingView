...
for df, cols in [(v2,["timestamp_utc","dp1_timestamp_utc","source_max_timestamp_utc"]),
...
for c in ["event_minutes_since_last_touch","event_minutes_since_last_cross",
    ...
    print(c.upper()+"_NEGATIVE=",int((s.dropna()<0).sum()))

print("\n=== REQUIRED FIELD COMPLETENESS ===")
required=["rth_vwap_at_decision","directional_vwap_distance_pct",
"directional_vwap_distance_min_from_dp1_pct","directional_vwap_distance_max_from_dp1_pct",
...
for c in required:
    print("MISSING_"+c.upper()+"=",int(v2[c].isna().sum()))
...
overall=(checks.get("PASS",False)
 and len(v2)==450491 and v2.decision_id.nunique()==450491
 and v2.event_id.nunique()==44627 and v2.symbol.nunique()==112
 and ids_v2==ids_struct==ids_out
 and dp1_after==0 and src_after==0 and neg_age==0
