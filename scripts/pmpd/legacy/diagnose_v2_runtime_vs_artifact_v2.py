...
for x in [csv, fp, zip_path]:
    print(
        ...
        "exists=", x.exists(),
        "mtime=", fmt_mtime(x),
        "size=", x.stat().st_size if x.exists() else None
    )
...
if zip_path.exists():
    with zipfile.ZipFile(zip_path, "r") as z:
        ...
        target = next(
            (n for n in names if n.endswith("decision_vwap_event_path_v2.csv")),
            None
        )
        if target:
...
print("core_module=", Path(core.__file__).resolve())
print("has_core_loader=", hasattr(core, "load_certified_partition"))

part = core.load_certified_partition(
    symbol="MSFT",
    year=2025,
    repo_root=root,
    verify_hash=True
...
live = core.build_symbol(part.dataframe.copy(), symbol="MSFT")
...
print(
    "live_has_event_vwap_loss_after_structural_clearance=",
    "event_vwap_loss_after_structural_clearance" in live.columns
)
...
for c in [
    ...
    print(
        c,
        "missing=",
        int(live[c].isna().sum()) if c in live.columns else "COLUMN_ABSENT"
    )
...
print(
    "roundtrip_has_source_max_timestamp_utc=",
    "source_max_timestamp_utc" in rr.columns
)
